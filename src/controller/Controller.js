...
class class1 {
  ...
  static GetPlatformFee = async (req, res) => {
    try {
      if (req.body.UserId && req.body.ShareId) {
        var PlatformFee = await PurchaseShare.find({
          User: req.body.UserId,
          Share: req.body.ShareId,
          PurchaseType: "Paid Sell"
        });
  ...
  static UserWithdrawal = async (req, res) => {
    try {
      ...
      const UniqueUserWithdrawal = Array.from(new Map(Data.map(item => [item.Name, item])).values());

      var a = {
        UserWithdrawal: UniqueUserWithdrawal,
        message: "Request Add Successfully",
        status: `${HTTP.SUCCESS}`,
      };
      res.status(HTTP.SUCCESS).json(a);
